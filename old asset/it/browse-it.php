...
a:active {
  color: grey;
	...
	<?php
		...
		$sql 	= $db->getRecFrmQry("SELECT * FROM assetdata WHERE 1 ".$condition." AND assetType LIKE 'IT%' ");
		$pages->items_total	=	count($sql);
		$pages->mid_range	=	9;
		$pages->paginate(); 
		 
		$userData	=   $db->getRecFrmQry("SELECT * FROM assetdata WHERE 1 ".$condition." AND assetType LIKE 'IT%' ORDER BY assetNo DESC ".$pages->limit."");
...
<!--			<div class="card-header"><i class="fa fa-fw fa-globe"></i> <strong>Browse IT Equipment</strong> <a href="add-it.php" class="float-right btn btn-dark btn-sm"><i class="fa fa-fw fa-plus-circle"></i> Add Asset</a></div>-->
            <div class="card-header"><i class="fa fa-fw fa-globe"></i> <strong>Browse IT Equipment</strong>
